...
You can run the Bitcoin Computer Node in your local machine connecting to regtest, testnet, or livenet (mainnet) network modes. When using regtest, the wallets must be funded. Therefore, you must first launch the ``db``, ``bitcoind``, and ``bcn`` services, and then fund the wallets.
...
When you run this command for the first time, the required software is downloaded and compiled; this can take several minutes. Subsequent starts will be much faster since they only need to start the server.

You will see the logs of the services that make up the Bitcoin Computer Node: a Litecoin Node called ``litecoind``, a database called ``db``, an api server called ``bcn``, and a process called ``sync``. The ``bcn`` and ```sync``` services depend on the ``db``. On first actions, both services will try to connect to ``db``. Until the database is up and running, messages indicating connection attempts will be logged.

Once the error messages stop emerging, you can run the tests:

```shell
yarn test
```

For local development, you will need to fund the test wallets using the following command. You can configure the wallets that are funded using the file ``chain-setup/ltc-regtest/topup-wallet.sh``. This will fund the default addresses only. To fund specific addresses of the regtest, update ``TEST_ADDRESS`` variable in environment variables. E.g. Address1;Address2 (; separates values).

```shell
yarn fund-ltc
```

For local development, the following are some default seed phrases to get funded:
...
<li>To ensure that Docker ECS integration is allowed to manage resources, enable your AWS credentials to grant access to AWS IAM permissions listed in <a href="https://docs.docker.com/cloud/ecs-integration/#install-the-docker-compose-cli-on-linux"> this page</a>.</li>
...
sudo ./aws/install
```
For Mac installation, follow the instructions in <a href="https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html">this</a> link.
...
4. Copy the file ``.env.aws.example`` into a file ``.env.aws`` in the root folder. Set the credentials already created and the AWS account name to the ``.env.aws`` environment file.
...
This project uses Amazon Elastic Container Registry (ECR) to hold the images for the Bitcoin Computer Node container. Also, the Amazon S3 Storage Service is utilized to hold the ``db_schema.sql`` file for the ``db`` service.

For a Docker and AWS integration, you must use two types of Docker contexts: one called "default" that will be used for both building the image in a local execution and pushing the image to ECR; the other one called "ECS" for deploying to AWS.

The following steps must be done in order to enable Docker to automatically deploy services into AWS.
...
List the Docker contexts to see if the new ECS context is created successfully.
```shell
docker context ls
```
...
8. Create a new Amazon S3 bucket, and upload the file ``db_schema.sql`` to the folder ``<bucket-name>/db/db_schema.sql``.

9. Update the ``.env.aws`` environment file with the full name of the bucket already created.
```shell
BUCKET=<bucket-name>
```
...
### The Deploy Command
Once the steps above are done, switch to the ECS context created in step 1, and run the following command to launch the Bitcoin Computer Node on AWS. The deployment will create several volumes in the Amazon Elastic File System (EFS) service. The volumes will not be removed unless you explicitly remove them; they can only be removed if they are not linked to any running task.
...
To remove a determined unlinked volumes in EFS, you can use the following command, providing the elastic file system id (efs-id) as the argument:

```shell
yarn docker-volume-rm-aws <efs-id>
```

You can first list all the volumes in EFS by using:
...
This command will automatically create a CloudFormation template, which will be deployed as an AWS ECS Fargate service. If everything is properly configured, the creation progress will be displayed on the console. The CloudFormation template can be seen as a stack in the AWS CloudFormation web dashboard.

In case you want to stop all the services and remove all the resources created in AWS from the CloudFormation template already created, run the following command:
...
Likewise, the CloudFormation template can be generated without deploying it to AWS. This can be useful for analyzing resources that will be created during the deployment. The template will be converted to a standard output by running the following command:
...
### Troubleshooting
1. Authentication error: an error like the following can be received when using aws console commands:
```shell
...
3. While the deployment is in the creation state, different types of problems can occur. One of the most common errors seen on the console is getting an EFS error code, which causes the process to be suspended and the deployment to be in a "delete in progress" state. If the error obtained is the following: 
```shell
TaskFailedToStart: ResourceInitializationError: failed to invoke EFS utils commands to set up EFS volumes: stderr: b'mount.nfs4: Connection reset by peer' : unsuccessful EFS utils command execution; code: 32
```
it is recommended to remove all the unlinked elastic file system volumes (if they do not contain relevant information), and run the deployment again.
...
* a fixed-low-fee (as defined below) per smart object creation and update. This fee applies to nested objects, so if you update a smart object on one of its sub-objects, you have to pay two fixed-low-fees. The fee does not apply to objects that can be garbage-collected.
...
If you have any questions, please contact us through our <a href="https://t.me/thebitcoincomputer">Telegram group</a> or <a href="https://twitter.com/TheBitcoinToken">Twitter</a>, or by email clemens@bitcoincomputer.io.
